...
@Component({
  selector: 'app-publish-irf',
  templateUrl: './publish-irf.component.html',
  styleUrls: ['./publish-irf.component.css']
})
export class PublishIrfComponent implements OnInit {

  _irfs: Array<IRFDetail>;
  ...
  constructor(private service: Service, private configService: ConfigService,
    ...
    this.columnDefs = [
      ...
      { headerName: 'IRF Type', field: 'IRF_Type' },
  ...
  ngOnInit() {
    ...
    this.toDate = '';
    this.errorMessage = '';
    this._irfs = new Array<IRFDetail>();
  ...
  openIRFDetails(id: number) {
    ...
    sessionStorage.setItem('irf-home', '/home/publish-irf');
    if (id >= 0) {
  ...
  loadSavedReport() {
    const reportParams = JSON.parse(sessionStorage.getItem("irf-publish-Report-Params"));
    if (reportParams) {
  ...
  getResult = () => {
    ...
    this.service.get(this.apiEndpoints.BASE_GET_APPROVED_IRF_ENDPOINT, options)
      .subscribe(c => {
        this._irfs = c;
  ...
  setReportParameters() {
    ...
    sessionStorage.setItem("irf-publish-Report-Params", JSON.stringify(reportParams));
  }
