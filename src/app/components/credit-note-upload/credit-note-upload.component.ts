...
@Component({
  selector: 'app-credit-note-upload',
  templateUrl: './credit-note-upload.component.html',
  styleUrls: ['./credit-note-upload.component.css']
})
export class CreditNoteUploadComponent implements OnInit {
  msg: string;
  indLoading: boolean;
  columnDefs: any;
  gridData: any;
  ...
  invoiceData: Array<CreditNote>;
  invoiceGridData: Array<CreditNote>;
  ...
  constructor(private service: Service, private configService: ConfigService,
    ...
    this.invoiceData = new Array<CreditNote>();
    ...
    this.uploader.onCompleteItem = (
      ...
      if (status == 200) {
        const data = new CreditNote();
        data.FileName = item.file.name;
        data.CreditNoteNumber = data.FileName.split('.')[0];
        data.InvoiceDocumentId = response;
    ...
    this.uploader.onCompleteAll = () => {
      ...
      this.service.post(this.apiEndpoints.BASE_GET_CREDITNOTE_DETAILS_ENDPOINT, this.invoiceData)
        .subscribe(c => {
    ...
    this.columnDefs = [
      ...
      {
        headerName: 'Credit Date', field: 'CreditDate', filter: 'agDateColumnFilter',
  ...
  clearData = () => {
    this.invoiceData = new Array<CreditNote>();
    this.invoiceGridData = new Array<CreditNote>();
  }

  attachToInvoice = () => {
    const selectedRows = this.gridApi.getSelectedRows();
    if (selectedRows.length === 0) {
      this.msg = 'Please select the Credit Note(s)';
    ...
    this.service.post(this.apiEndpoints.BASE_GET_CREDITNOTE_ATTACHMENT_ENDPOINT, selectedRows)
      .subscribe(c => {
        this.msg = c;
